...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const userData = { name, email: email.toLowerCase().trim(), password, address, phone, role: role || 'customer', location: location || { type: 'Point', coordinates: [0, 0] } };
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!allowedRoles[requestClientType] || !allowedRoles[requestClientType].includes(user.role)) {
        return res.status(403).json({ status: 'error', message: 'Tài khoản của bạn không có quyền truy cập.' });
    }
...
router.get('/me', verifyToken, async (req, res) => {
    try {
        ...
        // Vì middleware có thể không lấy shipperProfile, ta sẽ query lại cho chắc
        const user = await User.findById(req.user._id).select('name email phone address role shipperProfile');
